...
Da in diesem Beispiel (\seeref{lst:find-contacts}) keine bestimmten, sondern \emph{alle} Kontakte angezeigt werden sollen, wird der \lstinline|find|-Methode kein spezieller \lstinline|filter| übergeben (\linename 22). In der Oberfläche sollen der Name sowie der Anzeige-Name verfügbar sein, sodass der \lstinline|fields|-Parameter die entsprechenden Felder als Array beinhaltet (\linename 24). Bei erfolgreicher Ausführung der \lstinline|find|-Methode werden die zurückgegebenen Kontaktdaten als Parameter in Form eines Arrays, das die entsprechenden JavaScript-Objekte beinhaltet, an die \lstinline|onSuccess|-Methode übergeben und als Ergebnis zurückgeliefert.
...
Die in \autoref{lst:find-contacts} zurückgelieferten Daten werden vom ViewModel angefordert, sobald diese in der Oberfläche angezeigt werden sollen (\seeref{lst:ViewModel_Contacts}, \linename 14).
Dies geschieht in der Methode Methode \lstinline|getContacts()| des \glspl{view-model} (\autoref{lst:ViewModel_Contacts}, \linename 8).
