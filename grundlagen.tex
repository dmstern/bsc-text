...
Einige Gerätehersteller nutzen zum Ausführen dieser \glspl{app} die gleichen Betriebssysteme (wie bspw. \gls{android} oder \gls{win-phone}), die sich unter den verschiedenen Installationen auf dem Mobilgeräten nur in einigen herstellereigenen Anpassungen unterscheiden, grundlegend gibt es aber diverse Betriebssysteme für Mobilgeräte von unterschiedlichen Herstellern und Entwicklern.
...
Hierzu bietet beispielsweise \gls{intel} mit seinem \gls{app-framework} eine \gls{js}-Oberflächenbibliothek, der entsprechende Stylesheets für jede große Plattform zugrunde liegen. So wird diese Arbeit dem Entwickler abgenommen und die App soll sich so gut in das native Design einfügen \cite{IntelAppFramewor}.
