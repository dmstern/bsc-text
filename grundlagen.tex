...
\subsection{Lösungen für die plattformunabhängige App-Entwicklung}
...
\subsubsection{\gls{phonegap}} \label{sec:phonegap}

\gls{phonegap} ist ein \gls{opensource}-Framework von \gls{adobe} zur Erstellung von hybriden \glspl{app} und bildet damit die Grundlage für den hier explorierte Ansatz zur plattformunabhängigen \gls{app}-Entwicklung.
Die Software wurde ursprünglich von der Firma \gls{nitobi} entwickelt, die 2011 von \gls{adobe} aufgekauft wurde \cite{Adobe_Announces_Agreement_to_Acquire_Nitobi_Creator_of_PhoneGap}. 
...
Sowohl der Software-Kern \gls{apache} \gls{cordova} als auch dessen Distribution \gls{phonegap} von \gls{adobe} sind \gls{opensource}-Projekte \cite{PhoneGap_Cordova_and_whats_in_a_name}.



%TODO Wie wird PhoneGap verwendet und wie funktioniert es?
...
Mit seinem Online-Portal \gls{pg-build} bietet \gls{adobe} einen webbasierten Build-Service für \gls{phonegap}-Apps an, der den Bau-Prozesss, im Falle der hybriden \glspl{app} also die eigentliche Anbindung an plattformspezifische Toolkits auf nativer Ebene, auslagert und damit deutlich vereinfacht.
Wie in \autoref{fig:hybrid-apps-schaubild} beschrieben, muss dieser Entwicklungsschritt im Gegensatz zu den vorherigen\footnote{Also die Entwicklung des Codes sowie Tests.} und der Oberfläche) trotz (oder gerade wegen) des plattformunabhängigen Charakters mehrfach\footnote{Also einmal für jede Zielplattform} ausgeführt werden, um die vorher entwickelte Web-App in die Umgebung einer native App einzubetten (\seename\ \autoref{sec:hybrid-app}).
...
\includegraphics[
	...
	\caption
		[\Gls{pg-build}-Pakete]
		{Übersicht über die verschiedenen Bezahlpakete von \gls{pg-build}: Ab 9,99\,\$ im Monat können Entwickler bis zu 25 private \glspl{app} anlegen, in der kostenlosen Variante nur eine private, aber unbegrenzt öffentliche \cite{Adobe_PhoneGap_Build_Plans}.}
...
\includegraphics[
	...
	\caption[\Gls{pg-build}-Oberfläche]{Oberfläche des \gls{pg-build}-Portals: Detailansicht für eine Beispiel-\gls{app}. Hier können verschiedene Einstellungen vorgenommen werden, sowie der Code aus einem Repository aktualisert und die fertig gebaute \gls{app} per Download-Button oder \gls{qr} heruntergeladen werden.}
...
Bei der konkreten Verwendung von nativen Features muss jedoch teilweise wieder auf die Unterstützung durch die jeweiligen\glspl{plattform}geachtet werden. \todo{oder zumindest muss deklariert werden, was verwendet werden soll. -> checken!}
\end{comment}
