...
% => Seit Smartphones werden fast alle Web-Dienste (ja beinahe jede größere Website) auch parallel als App angeboten (Tagesschau / Wikipedia / etc.). Deshalb hier erstmal Web-App (Mobile Website).

\subsection{Mobile (native) App}
...
Seit 2013 bietet Google darüberhinaus die auf IntelliJ~IDE basierende und eigens für die Android-Entwicklung angepasste Entwicklungsumgebung \emph{Android~Studio} an, die bereits alle notwendigen Toolkits enthält. 

Nachdem der Code geschrieben ist, kann er kompiliert und zu einem lauffähigen Programm gebaut werden (\seename\ \autoref{fig:hybrid-apps-schaubild}). Anschließend kann die App in dem für die Zielplattform vorgesehene Dateiformat ausgeliefert werden.


% 	-> Kompilieren / bauen
% 	-> Deployment -> Verpacken
%	-> Nutzer kann App (Paket / Dateiformat) installieren

% iOS: 


\subsection{Vergleich / Schlussfolgerung -> Entwicklung von Hybriden Apps}
