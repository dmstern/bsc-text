...
So sichert sich Apple, nicht nur durch die kostenpflichtige Mitgliedschaft im \gls{ios-dev-prog}, sondern allein schon durch deren exklusive Platformunterstützung ihres eigenen Mobilbetriebssystems, auch mit jedem Entwickler einen neuen Kunden. \todo{darf man so was hier anmerken, oder lieber weg?}
...
Das \gls{cordova}-\gls{cli} bietet auch die Möglichkeit, alle verfügbaren Plugins zu durchsuchen, hinzuzufügen, aufzulisten und zu entfernen (\seeref{lst:cordova-plugin-search}, \ref{lst:cordova-plugin-add}, \ref{lst:cordova-plugin-ls} und \ref{lst:cordova-plugin-rm}).
...
\includebash{cordova-plugin-ls}{
	caption={Analog zum \lstinline|list|-Befehl für \glspl{plattform} (\seeref{lst:cordova-platform-list}) können auch Plugins des aktuellen Projekts aufgelistet werden.},
	}
