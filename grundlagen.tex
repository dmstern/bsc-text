...
%TODO MUSS AUF JEDEN FALL NOCH REIN!!! ::::::::::::::::::::::::
...
\subsection{Knockout} \todo{evtl. andere Überschriften} \label{sec:ko}
...
\image{without-ui-content-class}
	{width=1\textwidth}
	{Beispiel ohne class-Attribut}{Beispiel-Oberfläche wie in \autoref{fig:jqm}, ohne das class-Attribut in \autoref{lst:ko}, \linename 12.}
	{Eigener Screenshot.}
...
\gls{cordova} bietet von Haus aus eine ganze Reihe von \glspl{plugin} an, es können aber auch eigene entwickelt werden.\footnote{So bietet \zB \gls{adobe} mit seiner \gls{phonegap}-Variante noch einige weitere \glspl{plugin} wie beispielsweise den Zugriff auf einen angeschlossenen Barcode-Scanner an, die über die Grundausstattung von \gls{cordova} hinausgehen \cite{Cordova-Docs_CLI}.}
...
\paragraph{Verwendung von Plugins:}

Für die Installation und Dokumentation von \glspl{plugin} bietet \gls{apache} mit der \gls{plugins-registry} ein Online-Portal an, in der sich zur Zeit 229 \glspl{plugin} für das \gls{cordova}-\gls{framework} finden, von denen nur ein kleiner Teil von ca. 17 \glspl{plugin} von \gls{apache} selbst veröffentlicht wird  \cite{Cordova_Docs_Plugin_APIs}, der überwiegende übrige Teil stammt von verschiedenen Entwicklern, die ihre nach dem \gls{plugin-dev-guide} entwickelten \glspl{plugin} diesem Portal hinzufügen können \cite{Cordova_Plugin_Development_Guide}, und bieten häufig spezielle Unterstützung für bestimmte mobile Plattformen an \cite{Cordova_Plugin_Registry_viewAll}.
...
So beinhaltet beispielsweise das \emph{contacts}-\gls{plugin} neben dem zentralen \emph{Contact}-Objekt, das eine Instanz eines Eintrags im Adressbuch des Geräts repräsentiert, die Typen \emph{ContactName}, \emph{ContactField}, \emph{ContactAddress}, \emph{ContactOrganization}, die die Attribute des Contact-Objekt darstellen sowie das Konfigurationsobjekt \emph{ContactFindOptions}, das bestimmte Optionen für den Zugriff auf das Adressbuch kapselt und einen \emph{ContactError}, welcher bei Auftreten eines Fehlers beispielsweise bei der Suche nach Kontakten behandelt werden kann \cite{Cordova_Plugin_Registry_Contacts}.
...
\includehtml{cordova-plugin-contacts.js}{label=lst:cordova-plugin-contacts, caption={Beispiel für die Verwendung des \emph{Contacts}-\glspl{plugin} für die Erstellung eines neuen \emph{Contact}-Objekts \cite{Cordova_Plugin_Registry_Contacts}.},}
