...
Eine Problematik, die sich bei allen Ansätzen zur plattformunabhängigen App-Entwicklung stellt, sind die in \autoref{sec:native} erwähnten unterschiedlichen Designs und Designrichtlinien sowie unterschiedlichen Bedienkonzepte. Soll die hybride App also dem \gls{laf} einer nativen sehr nahekommen, so muss zu einem gewissen Grad die Benutzeroberfläche für jede Plattform beispielsweise mithilfe von Stylesheets separat angepasst werden.
...
Zusammen mit den Bibliotheken \gls{lime} und \gls{openfl} kann auf die gesamte \gls{flash}-\gls{api} zugegriffen, und durch integrierte Unterstützung für \gls{swf}-Dateien die Möglichkeit der Nutzung der \gls{flash-editing-environment} für alle gängigen Plattformen ermöglicht werden. 
Das Projekt richtet sich hauptsächlich an Spieleentwickler, die so die relativ einfach zu erstellenden \gls{flash}-Spiele auf alle Plattformen bringen können \cite{Haxe, OpenFL}.
...
%Durch die Abstraktion der Oberflächenbeschreibung kann mit den entsprechenden Schnittstellen so auf native Elemente des Betriebsystems zugeriffen werden, was vor allem für einen Fokus auf die User Experience und ein möglichst natives \gls{laf} interessant scheint.
...
\image{phonegap-build_create-public}
	...
	{Erstellung einer neuen App auf PhoneGap\,Build}
		{Dialog für die Erstellung einer neuen privaten App auf \gls{pg-build}. Links das Eingabefeld zum Eintragen eines Git-Repository-Links und rechts der Button zum Hochladen von Zip-Archiven.}
	{Eigener Screenshot.}
