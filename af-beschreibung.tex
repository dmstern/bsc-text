...
%TODO extend durch nameref ergänzen.

\usecase{Listen verwalten}
...
\usecase{Liste erstellen}
	\begin{description}
		...
		\item Weiter mit Liste benennen.
		\end{verlauf}
	\end{description}


\usecase{Liste umbenennen}
	\begin{description}
...
\usecase{Liste löschen}
	\begin{description}
		\actors{Initiiert vom Listeneigentümer}
		\begin{verlauf}
		
		\item Der Nutzer signalisiert dem System, dass er eine Liste löschen möchte.
		\item Weiter mit Objekt löschen.
		
		\end{verlauf}
	\end{description}

\usecase{Objekt löschen}
	Teil-Anwendungsfall. 
	\begin{description}
		\begin{verlauf}
...
\usecase{Tasks verwalten\,/\,Listeninhalt anzeigen}
	\begin{description}
		\actors{initiiert vom Listenmitglied}
		\begin{verlauf}
...
\usecase{Task bearbeiten}
	\begin{description}
		...
		Der Nutzer hat folgende Interaktionsmöglichkeiten:
			\begin{enumerate}
				\item Bild an Task hängen
				\item Task terminieren
				\item Task verorten
				\item Task zuweisen
				\item Task abschließen
				\item Erledigung abbrechen (Task zurückgeben)
				\item Task umbenennen
			\end{enumerate}
		
		\end{verlauf}
	\end{description}
	
\usecase{Bild an Task hängen}
	\begin{description}
		\actors{Initiiert vom Listenmitglied}
		\begin{verlauf}
...
\usecase{Erledigung abbrechen (Task zurückgeben)}
	\begin{description}
		\actors{Initiiert vom Task-Inhaber}
		\initcondition{Der Task wurde einem Nutzer zugewiesen, der dadurch zum Task-Inhaber wird.}
		\begin{verlauf}
		
			\item Verlauf wie bei Task zuweisen.
			\item Statt 3.: Der Nutzer wählt aus der Liste den Default-Wert „(kein)“ aus.
				
		\end{verlauf}
	\end{description}

\usecase{Task übernehmen}
	\begin{description}
		\actors{Initiiert vom  Listenmitglied}
		\begin{verlauf}
...
\usecase{Karte anzeigen}
	\begin{description}
		\actors{Initiiert vom Listenmitglied}
		\initcondition{Es wurde ein Ort für den betreffenden Task hinterlegt (Task verorten).}
		\begin{verlauf}
